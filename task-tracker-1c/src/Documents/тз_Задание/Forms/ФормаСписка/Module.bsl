...
Процедура СформироватьЗапросЗаданийПоАналитикам()
	...
	ШаблонТекстЗапроса = "ВЫБРАТЬ
		...
		|
		|ВЫБРАТЬ
		|	втЗаданияПоПрофилю.Ссылка,
		|	втЗаданияПоПрофилю.Номер,
		|	втЗаданияПоПрофилю.Дата,
		|	втЗаданияПоПрофилю.Основание,
		|	втЗаданияПоПрофилю.Состояние,
		|	втЗаданияПоПрофилю.СтатусВыполнения,
		|	втЗаданияПоПрофилю.Проект,
		|	втЗаданияПоПрофилю.Автор,
		|	втЗаданияПоПрофилю.ДатаЗавершения,
		|	втЗаданияПоПрофилю.ВидЗадания,
		|	втЗаданияПоПрофилю.Приоритет,
		|	втЗаданияПоПрофилю.Этап,
		|	втЗаданияПоПрофилю.Профиль,
		|	втЗаданияПоПрофилю.ТекущийИсполнитель,
		|	втЗаданияПоПрофилю.Наименование,
		|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.тз_ВидыАналитик.Состояние)
		|ИЗ
		|	втЗаданияПоПрофилю КАК втЗаданияПоПрофилю
		|;
		|
