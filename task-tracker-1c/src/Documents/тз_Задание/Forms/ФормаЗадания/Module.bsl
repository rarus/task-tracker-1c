...
Процедура СостояниеПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Состояние) Тогда
		СтатусВыполненияНовый = тз_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Состояние, "СтатусВыполнения");
		Если Объект.СтатусВыполнения <> СтатусВыполненияНовый Тогда
			Объект.СтатусВыполнения = СтатусВыполненияНовый;
		КонецЕсли;
		
		СтрокиСостояния = ТаблицаСостоянийПоПрофилю.НайтиСтроки(Новый Структура("Состояние", Объект.Состояние));
		Если СтрокиСостояния.Количество() > 0 Тогда
			Объект.ТекущийИсполнитель = СтрокиСостояния[0].Исполнитель;
		КонецЕсли;
	Иначе
		Объект.СтатусВыполнения = Неопределено;
		Объект.ТекущийИсполнитель = Неопределено;
	КонецЕсли;
...
Процедура Переадресовать(Команда)
	...
	ТекстОшибки = "";
	ИмяСправочникаПользователей = "Справочник."+ 
		тз_ОбщегоНазначенияВызовСервераТрекер.ПолучитьОпределяемыТипСтрокой("тз_Пользователи", ТекстОшибки);
	...
	ОткрытьФорму(ИмяСправочникаПользователей + ".ФормаВыбора",ПараметрыФормы,ЭтотОбъект,,,,ОписаниеОповещения);
...
Процедура ДобавитьОтложенныеФайлы(СсылкаЗадания = Неопределено, СсылкаКомментария = Неопределено)
	...
	Для Каждого ОтложенныйФайлСкрытый Из БуферФайловСкрытый Цикл
		...
		Если ОтложенныйФайлСкрытый.Значение.КомментарийВладелец = Неопределено Тогда
			тз_ОбщегоНазначенияВызовСервераТрекер.ЗаписатьОперациюБизнесСтатистики("ПрикреплениеФайлаКЗаданию");
		Иначе
			тз_ОбщегоНазначенияВызовСервераТрекер.ЗаписатьОперациюБизнесСтатистики("ПрикреплениеФайлаККомментарию");
		КонецЕсли;
...
Процедура ОбработатьВопросПоУдалениюФайлов(Результат, ДополительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ОбработатьУдалениеФайлаНаСервере(ТекущиеДанные.Файл);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьУдалениеФайлаНаСервере(Файл)
	УдалениеУспешно = тз_РаботаСФайламиВызовСервера.УдалитьФайлНаСервере(Файл);
